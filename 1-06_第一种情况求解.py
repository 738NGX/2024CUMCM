...
sale_data = pd.read_csv("data/结果/full_预期销售量.csv").iloc[:, 4:-1].values
...
model.addConstrs(S[i, j] <= sale_data[i, j] for i in range(656) for j in range(41))
